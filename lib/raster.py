...
__temperature_rasters__ = [f'chbio_{index}' for index in [1, 5, 6, 7, 8, 9, 10, 11]]
...
# raster_metadata = {'chbio_1': {'min_val': -10.7, 'max_val': 18.4, 'nan': -11.},
#                    'chbio_2': {'min_val': 7.8, 'max_val': 21.0, 'nan': 7.}}


all_rasters = tuple(sorted(raster_metadata.keys()))
...
class Raster(object):
    ...
    def _get_patch(self, item, cancel_one_hot=False):
        ...
        if self.size == 1:
            patch = self.raster[row_num, col_num].astype(float)
        ...
        # environmental tensor
            ...
            patch = self.raster[row_num-half_size:row_num+half_size,
                    col_num - half_size:col_num+half_size].astype(float)
            if self.one_hot and not cancel_one_hot:
                patch = np.array([(patch == i).astype(float) for i in self.unique_values])
            else:
                patch = patch[np.newaxis]
...
class PatchExtractor(object):
    ...
    position.
    """
    def __init__(self, root_path, size=64, verbose=False, resolution=1., temperature_rasters=__temperature_rasters__):
        self.root_path = root_path
        self.size = size

        self.verbose = verbose
        self.resolution = resolution
        self.temperature_rasters = temperature_rasters

        self.rasters = []

    def add_all(self, normalized=False, transform=None, add_T=0):
        """
        ...
        for key in sorted(raster_metadata.keys()):
            if 'ignore' not in raster_metadata[key]:
                self.append(key, normalized=normalized, transform=transform, add_T=add_T)

    def append(self, raster_name, add_T=0, **kwargs):
        """
        ...
        try:
            ...
            if add_T != 0:
                if raster_name in self.temperature_rasters:
                    r.raster = r.raster + add_T
