...
# random seed for replex purposes
SEED = 42
...
np.random.seed(SEED)
df = df.iloc[(np.random.sample(1000) * len(df)).astype('int'), :] # temporary restriction to a random subset
...
# load model
path = Path(r'C:\Users\fauren\PycharmProjects\cnn-sdm\rf.skl') # 'rf.skl'
print(f'Loading SKL model: {path}')
