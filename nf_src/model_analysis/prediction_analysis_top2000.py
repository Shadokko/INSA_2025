...
path2folder2analyze_top2000 = path2folder2analyze.parent / 'top2000'
...
path2outputfolder = Path(r'C:\Users\fauren\Documents_nosync\bac_a_sable_GP_databases\output\modele2000')

do_model_comparison = False
...
if do_model_comparison:
    ...
    category_orders = {'Modèle': ['rang rf_train_if_4_all_but_test_reg']}


# ===========================
# Analyse des modèles Top2000

concat_preds, melt_preds = concat_melt_predictions(path2folder2analyze_top2000)

# performances du modèle de référence sur le jeu de test
fig2000 = px.box(melt_preds,
              x='Observateur', y='Rang', color='Observateur', log_y=False,
             hover_data=['Nom_Valide', 'Observateur', 'Note'], template="seaborn",
              category_orders=category_orders, width=1150, height=800)
# fig2000.update_layout(title={'xanchor': 'center', 'yanchor': 'top'})
fig2000.update_layout(
    title="Performance du modèle 'de référence' à 2000 taxons"
          f"<br><sup>Ordonnée: rang de la vérité terrain (espèce effectivement observée) selon le classement de probabilités du modèle. "
          f"<br>Abscisse: note selon le modèle développé par Alain Poirel (-1 -> données non notées). "
          f"<br>Entrainement: toutes données d'Infloris, sauf données d'évaluation"
          f"<br>Modèles évalués sur données A. Poirel, J-M Tison et A. Mas ({len(concat_preds)} données InFloris)</sup>",
...
fig2000.update_layout(xaxis = dict(tickmode='array',tickvals=list(range(-1, 10))))
fig2000.update_layout(margin=dict(l=20, r=20, t=180, b=20))
fig2000.write_html(str(path2outputfolder / ('Perf_model_2000.html')))
fig2000.show()

# Visualisation des données mal classées Nicolas Faure
fig1 = px.scatter_mapbox(concat_preds.loc[(concat_preds.Observateur == 'Nicolas FAURE') &
...
fig1.update_layout(title="Données de Nicolas Faure mal classées par le modèle à 2000 espèces")
fig1.update_layout(mapbox_style="open-street-map")
fig1.write_html(str(path2outputfolder / ('Carte_donnees_mal_classees_Nicolas_Faure_modele2000.html')))
fig1.show()
...
concat_preds['test set'] = concat_preds.Observateur.isin(['Alain POIREL', 'Jean-Marc TISON', 'Anaïs MAS', 'Nicolas FAURE'])

# performances sur le jeu de train et test
fig2000 = px.box(df_filter(concat_preds, topN=50),
              x='Observateur', y='rank_ground_truth', color='test set', log_y=False,
             hover_data=['Nom_Valide', 'Observateur', 'Note'],
                 template="seaborn", width=1150, height=800)
# fig2000.update_layout(title={'xanchor': 'center', 'yanchor': 'top'})
fig2000.update_layout(
    title="Performance du modèle 'de référence' à 2000 taxons"
          f"<br><sup>Ordonnée: rang de la vérité terrain (espèce effectivement observée) selon le classement de probabilités du modèle. "
...
fig2000.update_layout(margin=dict(l=20, r=20, t=150, b=20))
fig2000.write_html(str(path2outputfolder / ('Perf_model_2000_all.html')))
fig2000.show()

# performances du modèle 2000 selon note
...
fig4.update_layout(xaxis = dict(tickmode='array',tickvals=list(range(-1, 10))))
fig4.update_layout(margin=dict(l=20, r=20, t=180, b=20))
fig4.write_html(str(path2outputfolder / ('Perf_model_2000_note.html')))
fig4.show()
